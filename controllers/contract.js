...
exports.create = asyncMiddleware(async (req, res, next) => {
  ...
  if (!(files && body)) error(400, 'payload or files missing')

  const representativesIds = body.representatives && body.representatives
    .split(',')
    .filter(id => isValid(id))

  if (!representativesIds) error(400, 'representatives field is empty or contains invalid user id(s)')
...
module.exports = {
  GET: async (req, res, next) => {
    ...
    if (contractId) {
      result = await Contract.findById(contractId).populate({
        path: 'representatives', select: 'personal_data -_id'
      })
    } else {
